		...
		<div class="col-menu">
			<ul class="menu-items">
				...
				<li class="last current"><a
					class="menu-txt iconfont icon-menu-bm" href="${applicationScope.url }/BankCardController/EnterBankCard">银行卡管理</a>
				</li>
			</ul>
		</div>
		<div class="col-main">
			...
			<c:when test="${fn:length(sessionScope.user.listBankCard)}==0">
				...
				<div class="content-hd">
					...
					<span id="isValid-show"></span>
						您当前未绑定银行卡 <a href="BankCardController/EnttoAddBankCard"
			...
			<c:otherwise> 
				...
				<div class="content-hd">
					<p class="tips">
					<span id="isValid-show"></span>
						您当前已绑定<span class="em">${fn:length(sessionScope.enterpriseUser.enterprise.bankcards)}</span>张银行卡 <a href="BankCardController/EnttoAddBankCard"
							id="applyNewCard">添加新的银行卡</a>
					<div id="LoginBox">
				...
				<div class="content-bd">
					<ul class="card-items">
						<c:forEach items="${sessionScope.enterpriseUser.enterprise.bankcards}" var="ca">
							<li class="df-show">
								...
								<div class="card-layout card-mid">
									...
									<div class="card-col-r">
										<a class="card-opt card-opt-hide"
											href="BankCardController/EntremoveBankCard?id=${ca.id}">删除</a>
									</div>
								</div>
								<div class="card-layout card-btm clearfix">
									...
									<div class="card-pay-l">
										<c:if test="${ca.isQuickPay==1 }">
											<a href="BankCardController/EntshutQuickPay?id=${ca.id}">关闭</a>
										</c:if>
										<c:if test="${ca.isQuickPay==0}">
											<a href="BankCardController/EntopenQuickPay?id=${ca.id}">开启</a>
										</c:if>
									...
									<div class="card-col-r">
										...
										<c:if test="${ca.isDefault==0}">
											<a class="card-opt"
												href="BankCardController/EntisDefault?id=${ca.id}">设为默认银行卡</a>
										</c:if>
									</div>
								</div>
							</li>
